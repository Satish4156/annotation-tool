...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            # PASS ERROR MESSAGE TO TEMPLATE INSTEAD OF PLAIN TEXT
...
def admin():
    ...
    escalated_reports = Report.query.filter_by(status="Escalated").count()
    qc_pending_reports = Report.query.filter_by(status="QC Pending").count()

    employees = Employee.query.all()
    employee_stats = []

    # Get today's range boundary to isolate daily progress
    today_start = datetime.combine(date.today(), time.min).strftime("%d-%m-%Y %I:%M %p")

    for employee in employees:
        # Counts items completed specifically TODAY
        completed_count = Report.query.filter(
            Report.assigned_to == employee.username,
            Report.status == "Completed",
            Report.employee_timestamp >= today_start
        ).count()

        pending_count = Report.query.filter(
            Report.assigned_to == employee.username,
            Report.status.in_(["Pending", "In Progress"])
        ).count()
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if existing_taxonomy:
            ...
            taxonomy = GlobalTaxonomy(tags=common_tags)
        ...
        for start in range(0, len(df), chunk_size):
            ...
            for _, row in chunk.iterrows():
                try:
                    ...
                    report_exists = Report.query.filter_by(report_id=report_id).first()
                    if report_exists:
                        continue

                    assigned_employee = employees[employee_index % len(employees)]
...
def employee():
    ...
    # Get today's calendar string for precise text-matching prefix
    today_prefix = date.today().strftime("%d-%m-%Y")

    # Count reports submitted TODAY that are finalized, sent to QC, or Escalated
    # Using .like() guarantees we capture any timestamp text beginning with today's date
    completed_count = Report.query.filter(
        Report.assigned_to == username,
        Report.status.in_(["Completed", "QC Pending", "Escalated", "Mismatch", "QC Passed"]),
        Report.employee_timestamp.like(f"{today_prefix}%")
    ).count()
    ...
    if taxonomy and taxonomy.tags:
        raw_tags = taxonomy.tags.replace('\n', ',')
    ...
    # SIMILAR REPORTS
    # ================================================
    similar_reports = Report.query.filter(Report.selected_tags != None).limit(20).all()
...
def submit(id):
    ...
    username = session['username']
    report = Report.query.filter_by(id=id, assigned_to=username).first()

    if report:
        selected_tags = request.form.getlist('tags')
        action = request.form.get('action')

        # Shared processing setup for timestamps
        report.employee_name = username
        report.employee_timestamp = datetime.now().strftime("%d-%m-%Y %I:%M %p")
        ...
        if action == "escalate":
            ...
            team_lead = Employee.query.filter_by(role="team_lead").first()
            if team_lead:
        ...
        report.selected_tags = ", ".join(selected_tags)
        sample = random.randint(1, 100)

        if sample <= 5:
            report.qc_required = True
            employee = Employee.query.filter_by(username=report.assigned_to).first()
            if employee:
...
# =====================================================
# ESCALATE REPORT (Direct Button Handler fallback)
# =====================================================
...
def qc_dashboard():
    reports = Report.query.filter(Report.status == "QC Pending").all()
    mismatch_reports = Report.query.filter_by(mismatch=True).all()
    return render_template(
        'qc_dashboard.html',
        reports=reports,
...
def qc_submit(report_id):
    ...
    report.qc_timestamp = datetime.now().strftime("%d-%m-%Y %I:%M %p")

    employee_tags = []
    if report.selected_tags:
        employee_tags = [x.strip() for x in report.selected_tags.split(',') if x.strip()]

    qc_tags = [x.strip() for x in selected_tags if x.strip()]
...
def team_lead():
    ...
    reports = Report.query.filter_by(status="Escalated").all()
    return render_template('team_lead.html', reports=reports)
...
def production_dashboard():
    ...
    total_reports = Report.query.count()
    completed_reports = Report.query.filter(Report.status.in_(["Completed", "QC Passed"])).count()
    pending_reports = Report.query.filter(Report.status.in_(["Pending", "In Progress"])).count()
    mismatch_reports = Report.query.filter_by(mismatch=True).count()
...
def quality_dashboard():
    total_qc_reports = Report.query.filter(Report.qc_required == True).count()
    mismatch_reports = Report.query.filter_by(mismatch=True).count()
    qc_pending = Report.query.filter_by(status="QC Pending").count()
    completed_reports = Report.query.filter_by(status="Completed").count()
...
def mismatch_dashboard():
    ...
    reports = Report.query.filter_by(mismatch=True).all()
    return render_template('mismatch_dashboard.html', reports=reports)
